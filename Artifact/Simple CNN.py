...
e = Embedding(90000, 200, weights=[embedding_matrix], input_length=200, trainable=False)
model_cnn_01.add(e)
model_cnn_01.add(Conv1D(filters=100, kernel_size=2, padding='valid', activation='relu', strides=1))
model_cnn_01.add(GlobalMaxPooling1D())
model_cnn_01.add(Dense(100, activation='relu'))
model_cnn_01.add(Dropout(0.2))
model_cnn_01.add(Dense(50, activation='relu'))
model_cnn_01.add(Dropout(0.2))
...
# ************---------- fit model CNN03---------------------*********************
batch_size = 30
model_cnn_01.fit(X_train, Y_train, epochs=100, verbose=1, batch_size=batch_size,validation_data= (X_test, Y_test),callbacks=[early_stop1])
# ************---------------------------------------------------------------------
...
encoder.fit(Y_test)
model_cnn_01_y_test = encoder.transform(Y_test)
model_cnn_01_y_test = np_utils.to_categorical(model_cnn_01_y_test)
# ____________________________ PREDICTING O TEST TEST _____________________________

#analyze the results
model_cnn_01_y_pred = model_cnn_01.predict(X_test)
model_cnn_01_y_prob = model_cnn_01.predict_proba(X_test)

model_cnn_01_score, model_cnn_01_acc = model_cnn_01.evaluate(X_test, Y_test, verbose=2, batch_size=batch_size)
print("score", model_cnn_01_score)
print("Accuracy", model_cnn_01_acc)
# ____________________________Classification report_______________________________________________________

print(classification_report(Y_test, np.argmax(model_cnn_01_y_pred, axis = 1)))
...
model_cnn_01_y_prob_F1 = cat_pred(model_cnn_01_y_prob)
model_cnn_01_y_pred_F1 = cat_pred(model_cnn_01_y_pred)
model_cnn_01_f1_score = f1_score(Y_test, model_cnn_01_y_prob_F1, average='macro')
model_cnn_01_f1_micro_ovo = f1_score(Y_test, model_cnn_01_y_pred_F1, average='micro')

#-----------------------Im Able to print this one ------------------------------------------------------------#

macro_roc_auc_ovo = roc_auc_score(model_cnn_01_y_test, model_cnn_01_y_prob, multi_class="ovo", average="macro")
macro_roc_auc_ovr = roc_auc_score(model_cnn_01_y_test, model_cnn_01_y_pred, multi_class="ovr", average="macro")
weighted_roc_auc_ovo = roc_auc_score(model_cnn_01_y_test, model_cnn_01_y_prob, multi_class="ovo", average="weighted")
weighted_roc_auc_ovr = roc_auc_score(model_cnn_01_y_test, model_cnn_01_y_pred, multi_class="ovr", average="weighted")
...
model_cnn_01_fpr = dict()
model_cnn_01_tpr = dict()
model_cnn_01_roc_auc = dict()
n_classes = 3
for i in range(n_classes):
    model_cnn_01_fpr[i], model_cnn_01_tpr[i], _ = roc_curve(model_cnn_01_y_test[:, i], model_cnn_01_y_pred[:, i])
    model_cnn_01_roc_auc[i] = metrics.auc(model_cnn_01_fpr[i], model_cnn_01_tpr[i])

# Compute micro-average ROC curve and ROC area
model_cnn_01_fpr["micro"], model_cnn_01_tpr["micro"], _ = roc_curve(model_cnn_01_y_test.ravel(), model_cnn_01_y_pred.ravel())
model_cnn_01_roc_auc["micro"] = metrics.auc(model_cnn_01_fpr["micro"], model_cnn_01_tpr["micro"])


# First aggregate all false positive rates
model_cnn_01_all_fpr = np.unique(np.concatenate([model_cnn_01_fpr[i] for i in range(n_classes)]))

# Then interpolate all ROC curves at this points
model_cnn_01_mean_tpr = np.zeros_like(model_cnn_01_all_fpr)
for i in range(n_classes):
    model_cnn_01_mean_tpr += interp(model_cnn_01_all_fpr, model_cnn_01_fpr[i], model_cnn_01_tpr[i])

# Finally average it and compute AUC
model_cnn_01_mean_tpr /= n_classes

model_cnn_01_fpr["macro"] = model_cnn_01_all_fpr
model_cnn_01_tpr["macro"] = model_cnn_01_mean_tpr
model_cnn_01_roc_auc["macro"] = metrics.auc(model_cnn_01_fpr["macro"], model_cnn_01_tpr["macro"])

# Plot all ROC curves
plt.figure()
lw = 3
plt.plot(model_cnn_01_fpr["micro"], model_cnn_01_tpr["micro"],
         label='micro-average ROC curve (area = {0:0.2f})'
               ''.format(model_cnn_01_roc_auc["micro"]),
         color='deeppink', linestyle=':', linewidth=4)

plt.plot(model_cnn_01_fpr["macro"], model_cnn_01_tpr["macro"],
         label='macro-average ROC curve (area = {0:0.2f})'
               ''.format(model_cnn_01_roc_auc["macro"]),
         color='navy', linestyle=':', linewidth=4)

from itertools import cycle
colors = cycle(['aqua', 'darkorange', 'cornflowerblue'])
for i, color in zip(range(n_classes), colors):
    plt.plot(model_cnn_01_fpr[i], model_cnn_01_tpr[i], color=color, lw=lw,
             label='ROC curve of class {0} (area = {1:0.2f})'
             ''.format(i, model_cnn_01_roc_auc[i]))
...
plt.title('ROC CURVE MODEL Simple CNN , EPOCHS = 100, Batch size')
